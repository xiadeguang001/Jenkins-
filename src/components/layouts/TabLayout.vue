  ...
  export default {
    ...
    data() {
      return {
        ...
        menuVisible: false,
        menuItemList: [
          { key: '4', icon: 'reload', text: 'リフレッシュ' },
          { key: '1', icon: 'arrow-left', text: ' 左側閉じる' },
          { key: '2', icon: 'arrow-right', text: '右側閉じる' },
          { key: '3', icon: 'close', text: '他閉じる' }
        ],
        reloadFlag:true
      }
    },
    ...
    methods: {
      ...
      changeTitle(title) {
        let projectTitle = "国内航空貨物システム（DACS）"
        // 首页特殊处理
        if (this.$route.path === indexKey) {
      ...
      remove(key) {
        ...
        if (this.pageList.length === 1) {
          this.$message.warning('これは最後のページです。閉じることができません')
          return
        }
